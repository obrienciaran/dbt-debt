...
The tool covers BigQuery, Snowflake, Redshift, and Databricks, runs as a standalone Python
command-line program, and works at both the model and the column level. BigQuery, Snowflake,
and Redshift are validated against live data. Each warehouse section lists what is confirmed
...
dbt_debt/
  ...
  consumption/           # ask the warehouse what was actually used
    ...
    databricks.py        # the Databricks version (the only file that imports the SQL connector,
                         #   also an optional extra; see the Databricks section)
    ...
    jobs.py              # the BigQuery queries (query log, table list, first-seen) and the
                         #   warehouse-neutral row parsers every client feeds
    ...
    databricks_queries.py # the Databricks system-table queries (lineage, query history, tables)
...
settings ─┐
          ...
          │     • each table's live storage bytes (Snowflake and Redshift only; on BigQuery and
          │       Databricks sizes come from catalog.json)
...
lazily, so a BigQuery user never installs anything Snowflake-related, and vice versa. The
Snowflake connector is the `[snowflake]` optional extra.
...
exact per-relation query counts, the dbt exclusion, orphan discovery through the case
normalization, the first-seen guard, and exit 0 end to end. One catalog detail came out of that
run: dbt-snowflake writes the table size under the `bytes` stats key ("Approximate Size"), not
BigQuery's `num_bytes`, so the catalog reader checks both (views carry no stats and report 0).
...
- **Usage comes from `ACCOUNT_USAGE.ACCESS_HISTORY`** (`direct_objects_accessed`, flattened to
  ...
  to `QUERY_HISTORY` for the SELECT/success/window/dbt-exclusion filters. Usage never falls back
  to sqlglot-parsing `query_text`: a silently unparseable query would erase evidence of use and
  produce a false "unused" verdict, the one failure mode this tool must never have.
  ACCESS_HISTORY needs Enterprise edition and IMPORTED PRIVILEGES on the `SNOWFLAKE` database.
  On Standard edition the preflight stops the scan, mirroring the `jobs.listAll` stance.
- **First-seen comes from `ACCOUNT_USAGE.TABLES` including dropped incarnations**, taking
  ...
  comes from JOBS rather than TABLES. *Confirmed live:* a mart replaced, then dropped outright
  and rebuilt, keeps one row per incarnation and reports its original creation date throughout,
  so it files under too-new rather than missing-first-seen; and a brand-new dead model is set
...
- **A dead node with no first-seen row is set aside, not judged** (*decided 2026-07-10*).
  ...
  unused-derived figure. The rare band gets the same protection. BigQuery is untouched: there
  first-seen comes from JOBS, so a missing row means zero jobs all window, the strongest unused
  signal there is.
- **The dbt exclusion assumes dbt's query-comment lands in `query_text`** (it does on BigQuery).
  ...
  *Confirmed live:* dbt's builds are excluded from usage counts.
- **Table sizes come live from `ACCOUNT_USAGE.TABLE_STORAGE_METRICS`** (same grant as the rest
  of ACCOUNT_USAGE, so no new permission), replacing catalog sizes where a row exists, giving
  warehouse truth with no `dbt docs generate` needed. Active bytes drive the size and
  reclaimable figures; the time-travel and fail-safe bytes still billed for the relation are
  summed over every incarnation, dropped ones included, and shown next to each unused table.
  Reported as bytes rather than dollars, since storage rates vary by contract and region. Two
  caveats seen live: dbt builds transient tables by default, which keep no fail-safe copies, so
  those figures are often zero; and the view lags like the rest of ACCOUNT_USAGE, so a brand-new
  table falls back to its catalog size until a row appears.
- **Orphans** read one `<database>.INFORMATION_SCHEMA.TABLES` filtered by lowercased schema
  name, one query rather than BigQuery's per-dataset union, because Snowflake's information
  schema spans the database. Uppercase identifiers normalize away because every relation key is
...
`[redshift]` optional extra, `redshift-connector`), and the sqlglot `redshift` dialect. The SYS
system views the scan reads work on both Serverless workgroups and provisioned clusters. The
adapter is built from AWS's published schemas, pinned by tests, and its core loop is validated
live against a Serverless workgroup via `demo_redshift/`: the preflight, per-relation usage
counts, dbt-build exclusion, the orphan with its direct-query evidence, the too-new guard, the
result-cache join, SVV_TABLE_INFO sizes, the stale-source skip note, the unused declared source
with its one direct query, and the table-hygiene check returning empty, exit 0 end to end.
...
- **Usage comes from `SYS_QUERY_DETAIL` scan steps joined to `SYS_QUERY_HISTORY`.**
  SYS_QUERY_HISTORY alone has no per-table column, but each scan step names the relation it read
  as a fully qualified `database.schema.table`, the engine-metadata analogue of
  `referenced_tables` and ACCESS_HISTORY, so usage never comes from parsing query text. The
  optimizer's own temp tables (`volt_tt_*`) and the system schemas are filtered out. *Confirmed
  live:* the three-part `table_name` shape, and counts matching the planted queries exactly.
- **Result-cache hits are followed to the originating query.** A cache-hit SELECT runs no scan
  ...
  disuse. The usage join goes through
  `COALESCE(NULLIF(result_cache_query_id, 0), query_id)`, so a cached repeat counts as use of
  the tables the original query read. *Confirmed live:* an identical repeat came back with
  `result_cache_hit` true and its source's query id, and both runs counted.
- **The preflight checks visibility, not access.** Every user can select from the SYS views;
  ...
  `SYSLOG ACCESS UNRESTRICTED`. An access-error probe would therefore pass wrongly, so the probe
  queries `SVV_USER_INFO` for the current user's own visibility and treats an empty result as
  the missing permission. *Confirmed live* as a superuser; the regular-user empty-result path is
  pinned by tests only.
- **dbt builds surface under `__dbt_tmp` names, folded back in first-seen.** dbt-redshift builds
  each table as `<name>__dbt_tmp` and renames it into place, and the rename is DDL that
  SYS_QUERY_DETAIL records under no name (found live), so the first-seen query strips that
  suffix and lets the tmp incarnation date the final relation. Without it, dbt-built tables get
  no first-seen row and the too-new guard never protects them. One residual edge, also found
  live: a CTAS whose plan scans no base table (the demo's recursive-CTE time spine) is logged as
  plain `DDL` with no named steps at all, so such a relation still has no first-seen date and
  can be called unused before `--min-age-days` passes.
- **SYS timestamps arrive naive.** The views report UTC as `timestamp without time zone`, so the
...
- **Table sizes come live from `SVV_TABLE_INFO`** (`size` in 1 MB blocks, converted to bytes),
  replacing catalog sizes where a row exists, like Snowflake's storage metrics. Redshift has no
  time-travel or fail-safe retention, so those fields stay zero and the report shows no
  retained-bytes breakdown. SVV_TABLE_INFO omits empty tables, which keep their catalog size.
  *Confirmed live,* with one reading trap: a 6-row table reports 35 MB, since the 1 MB-block
  floor per column slice makes tiny tables look large. The ranking is still right.
...
- **The stale-source check is skipped with a note.** Redshift exposes no last-data-received
  metadata (no `last_altered`, no `__TABLES__` analogue), and inferring it from the query history
  would both breach the retention window and break the "metadata, never query history" rule, so
  the check is gated off in the CLI the way the partitioning check is BigQuery-only.
...
- **Connection comes from `REDSHIFT_*` environment variables** (`HOST`, `USER`, `PASSWORD`,
  ...
  inferred from the models, like `--project` elsewhere. Because the endpoint lives in an env var
  rather than a flag, `REDSHIFT_HOST` joins the scan-cache key, so two workgroups sharing a
  database name never serve each other's cached rows.
- **A Redshift-only table-hygiene check reads the `SVV_TABLE_INFO` maintenance columns**
  (`verdict/redshift_hygiene.py`), the counterpart of the BigQuery-only partitioning check. It
  flags tables of 1 GiB or more (at most 20) whose `unsorted` region is 20% or larger (scans
  stop pruning until VACUUM runs), whose `stats_off` is 10 or more (stale planner statistics;
  ANALYZE resets it), or whose `skew_rows` ratio is 4 or more (one slice becomes the
  bottleneck). Those are AWS's own maintenance lines, held as module constants rather than
  flags. Ranking is by the bytes user queries scanned from each table (stored size as the
  fallback), so the top entry is the fix that saves the most. The columns can be NULL (no
  sortable data, zero rows, DISTSTYLE ALL) and parse as 0, which never trips a threshold. An
  empty result renders nothing, and that is the healthy state: on Serverless and modern
  provisioned clusters automatic vacuum and analyze keep every figure near zero, which is what
  the live demo shows. A flagged specimen has not been observed live, since planting one is
  timing-dependent at best. The rejected alternative, ranking raw sort/distribution-key
  declarations the way the partitioning check ranks `partition_by`, stays rejected: Redshift
  assigns both automatically, so a bare declaration is not debt, only measured neglect is.
  Review-only, one extra system-view read, and it never feeds any unused figure.

## 🧱 Databricks

The adapter uses the existing `WarehouseClient` seam and lazily imports the optional SQL
connector. Authentication comes from `DATABRICKS_HOST` (or `DATABRICKS_SERVER_HOSTNAME`),
`DATABRICKS_HTTP_PATH`, and an optional `DATABRICKS_TOKEN`; the normalized host and HTTP path
isolate cache entries without including credentials. The preflight reads both
`system.access.table_lineage` and `system.query.history`, and partial access fails closed,
because a relation cannot safely be called unused from only one of those sources. Relation
inventory is separately optional and reads `system.information_schema.tables`.

- **Usage follows a conservative hybrid.** Lineage with a `statement_id` joins successful
  `SELECT` history through `cache_origin_statement_id`, which equals the query's own ID for an
  uncached query and the originating ID for a cache hit. That permits exact dbt query-comment
  exclusion while retaining cache repeats wherever the ID is available. Lineage without a
  joinable statement ID counts as usage only when it has a source and no table or path target;
  source-to-target events are treated as probable build lineage and omitted. Missing or
  encrypted statement text also counts as usage. The asymmetry is deliberate: it permits false
  activity rather than false "unused" verdicts.
- **First-seen is the earliest source or target event still retained in table lineage.** Unity
  Catalog `created` is not used, because dbt table materializations reset it (confirmed live: a
  direct `CREATE OR REPLACE TABLE` preserved `created` while a dbt materialization changed it).
...
What live validation covered, on a controlled temporary schema and then on Free Edition: direct
and view reads, CTAS and replacement, an exact repeated query, a dbt run, and a dbt test on a
SQL warehouse, plus equivalent reads and writes on serverless and classic job compute. Fresh
system-table rows appeared after roughly 10 to 20 minutes. SQL-warehouse lineage IDs matched
query-history statement IDs, uncached rows had `cache_origin_statement_id` equal to their own
statement ID, dbt's JSON query comment was visible and excluded, and an end-to-end scan reported
three active models and the deliberately unread one as unused, with its removable tests and
catalog storage bytes. Two repeated warehouse reads reused an earlier result and both counted.
Unity Catalog reports managed tables as `MANAGED` and views as `VIEW`, and dbt-databricks 1.11
catalog output uses the already-supported `bytes`, `rows`, and `has_stats` keys. Representative
compiled model SQL, CTAS, view, and query statements parse with sqlglot's `databricks` dialect
(`DESCRIBE DETAIL` does not, which is irrelevant while the disabled column path never parses
it). Column-lineage rows were visible for the SQL-warehouse workload, but column mode stays
disabled because one controlled workload does not prove cross-compute coverage, and
classic-compute coverage remains documentation-based. One earlier principal could read
`system.information_schema.tables` but lacked `USE SCHEMA` on `system.query` and
`system.access`, and the scan stopped at the preflight as designed rather than attempting a
weaker verdict.
...
query count, last-queried date, size, and the bytes its queries scanned, and it never folds into
any unused-derived figure, because observed use is use. The band is ranked by scanned bytes
first (stored size as the fallback), so the expensive-but-rarely-used model, the strongest
deprecation candidate, tops the list. The too-new guard applies to the band the same way it
applies to the dead set, since a model created mid-window hasn't had a full window to accumulate
queries. The usage counts were always fetched; this band just stops discarding them.
...
than bill them, and no usage verdict ever depends on them. The same figure backs the
direct-query evidence on unused declared sources and on orphaned relations, where the
still-queried ones (the dangerous-to-drop ones) rank first. Validated live on both warehouses:
counts, dates, and scanned bytes match the queries run against the demos, and the ranking
follows bytes rather than query count. One trap the live run surfaced: a query run in a
different GCP project than the scan targets never appears in that project's
`INFORMATION_SCHEMA.JOBS`, which is exactly the project-scoping the permission check exists for.

Three hygiene stats ride along, all computed from dbt's own files with no warehouse call (the
Redshift table-hygiene check is the one exception, costing a single cheap `SVV_TABLE_INFO`
read). `verdict/coverage.py` counts models with at least one test and models and columns with
descriptions (the column denominator prefers the catalog's physical columns, and the sentence
says which universe was used). `verdict/drift.py` reports documentation drift: a column declared
in a model's YAML that no longer exists in the built relation per `catalog.json`. Nodes absent
from the catalog are skipped, since an unknown physical schema is not drift, and the report
notes that a stale catalog can false-positive, pointing at `dbt docs generate`.
...
`cluster_by` was declared and rebuilt, small tables never fired it, and with two such tables
planted the list ordered them by scanned bytes (875.5 MB above 291.8 MB).
...
## 🕵️ Orphans and the source findings
...
access to that dataset, while the one region-wide list needs a stronger, project-wide grant that
even an Owner can be refused (confirmed live on BigQuery). If we can't read the lists, we skip
this finding with a warning and the scan still succeeds. The usage rows already fetched attach
direct-query evidence to each orphan: a queried orphan is still read directly and is dangerous
to drop, so the still-queried ones rank first.

An **undeclared source** is a table a model reads from that dbt has no record of. It should be
declared as a `source()`. We find these by reading the model's own SQL (`references.py`), so the
check needs no warehouse access at all and works even when we can't list the warehouse tables.
...
already fetched attach evidence to each entry. A zero query count means the declaration is dead
weight; a non-zero count means people query the raw table directly and it may be worth modelling
instead of deleting. Like the rarely-used band, this is a review list and never feeds the
unused-model figures.

A **stale source** (`verdict/staleness.py`) is a declared source whose table has received no new
data for more than `--stale-source-days` (default 30; `0` disables). That usually means the
loader upstream of dbt has stopped, which no usage figure can catch. The last-data date comes
from warehouse metadata, never from query history, and a source with no metadata row is skipped,
since absent metadata is not evidence. On BigQuery each source dataset's legacy `__TABLES__`
table supplies `last_modified_time` (updated by loads and streaming writes) and needs only
dataset read access, the same optional grant as orphans; a missing grant skips the check with a
warning. Both halves are confirmed live on `demo_bq`, degrade path included, with the rest of
the scan unaffected and exit code 0. On Snowflake the check reads
`ACCOUNT_USAGE.TABLES.last_altered` (already required for first-seen, so no new grant), taking
`MAX` over the live rows. One caveat observed live there: a bare `COMMENT ON TABLE` moves
`last_altered` immediately, so any DDL resets the staleness clock and the check can under-report
staleness, never invent it. The verdict is pure (sources and a date map in, a list out) and,
like every review band, feeds no unused figure.
...
always counts as an undeclared source, so it never shows up as an orphan. (Seeds, snapshots, and
sources are taken out too, so a seed sitting next to your models is never flagged.)
...
Exposures carry one extra verdict of their own. An exposure whose every model dependency is dead
is reported as **likely dead** itself: nothing the dashboard reads was queried in the whole
window, so the strongest explanation is that nobody opens the dashboard either. The affected and
likely-dead lists are mutually exclusive (an exposure with some but not all upstream models dead
stays "affected, review"), non-model dependencies are ignored for the all-dead rule, and because
the dead set already excludes too-new and rarely-used nodes, an exposure over those is never
flagged.
...
consumer in between), so the report names the cause on every line and the detail view walks the
chain from the saved query down to the model. A dead column that a semantic model names in an
entity, dimension, or measure `expr` is *blocked* rather than consumed (`verdict/semantic.py`
...
expects, and the entity/dimension/measure `expr`-with-name-fallback parsing produces the right
`column_refs`, with no parser change needed. A live scan itemizes all three consumers under a
dead model. One inference remains, flagged per our rule: expression parsing falls back to "no
column refs" when sqlglot can't read an expr. One practical note: dbt refuses to parse a project
...
just parse time, and `INFORMATION_SCHEMA.TABLES.creation_time` resets on every `dbt run` (tables
and views are dropped and recreated) and lives on the permission-fragile orphan path. First-seen
instead rides on `JOBS_BY_PROJECT`, the one grant already hard-required, so there is no new
degradation mode. (Inference to confirm live: that CTAS and dbt builds populate
...
The warehouse steps of a scan can take a while, so two comforts exist for them. Both step aside
cleanly rather than become things the tool can't run without.

The **spinner** (`report/spinner.py`) shows a "working..." line during the slow warehouse steps
so a long scan doesn't look hung. It uses only the standard library, writes to the error stream
(reports, JSON, and `-o` output are never touched), only animates in a real terminal, and draws
plain text with no special terminal codes, so it is safe on older Windows consoles and inert
during tests.

The **cache** (`consumption/cache.py`) saves the slow warehouse round-trips (used tables, query
text, and the table lists) as JSON files so repeat runs are fast. It wraps the client rather
than changing it, which keeps the "only one file touches each warehouse SDK" rule and lets the
fake client exercise it in tests. Files are keyed by the things that change the answer
(warehouse, project, region, lookback window, and which queries count as dbt's) and deliberately
never by your dbt project. The permission preflight is never cached, because permissions can
change and that check is load-bearing.
...
files hold warehouse metadata (table names, usage counts, sizes), so the directories are created
owner-only (0700) and each entry is written atomically to a 0600 file via a private temp file
and a rename, which also stops another local user pre-planting files or symlinks at the
predictable paths. For the same reason, the finished text report strips terminal control bytes
before printing, so a crafted model name or file path in `manifest.json` cannot inject escape
sequences that recolour or overwrite report lines.
...
disk and so outlives the terminal session). A later flag-less run judges each entry against the
entry's own TTL; passing `--cache-ttl` explicitly overrides the stored values for that run, in
both directions (it can extend or force-shorten). Because the TTL lives in the entries, clearing
the cache also clears the remembered TTL; the next scan writes fresh entries at the default
(1 hour) unless the flag is passed again.

Past its time-to-keep an entry counts as a miss, and expired files are swept at the start of the
next scan by our own code, so cleanup behaves the same on every OS and nothing else has to clean
the cache directory. Clearing by hand has two forms. `dbt-debt --clear-cache` deletes the whole
cache folder and stops; `dbt-debt scan --clear-cache` clears this project's results and then
scans fresh. `--no-cache` skips the cache entirely; it neither reads nor writes.
